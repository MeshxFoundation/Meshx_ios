...
//    self.encapsulator.delegete = self;
    ...
    UInt32 val = 1;
    AudioQueueSetProperty(mAQRecorder->Queue(), kAudioQueueProperty_EnableLevelMetering, &val, sizeof(UInt32));
    
    self.timerLevelMeter = [NSTimer scheduledTimerWithTimeInterval:0.3 target:self selector:@selector(updateLevelMeter:) userInfo:nil repeats:YES];
...
- (void)updateLevelMeter:(id)sender {
    if (self.delegate) {
        ...
        AudioQueueGetProperty(mAQRecorder->Queue(), kAudioQueueProperty_CurrentLevelMeterDB, &levelMeter, &levelMeterSize);
        
        float peakPower = levelMeter.mAveragePower;
...
//        NSLog(@"=======volume:%lf====",peakPowerForChannel);
//        NSLog(@"========%lf=======",levelMeter.mPeakPower);
        
        if ([self.delegate respondsToSelector:@selector(levelMeterChanged:)]) {
            

            [self.delegate levelMeterChanged:levelMeterStates[0].mAveragePower];
        }
