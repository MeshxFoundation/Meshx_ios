...
@property (nonatomic, weak, readwrite) FacePanel *emotionManagerView;
 ...
 */
- (void)exChangeMessageDataSourceQueue:(dispatch_block_t)queue;
 ...
 */
- (void)exMainQueue:(dispatch_block_t)queue;
...
//    [self scrollToBottomAnimated:YES];
    ...
    [weakSelf exMainQueue:^{
        ...
        weakSelf.isReloading = YES;
        [self.messageTableView reloadData];
         weakSelf.isReloading = NO;
...
- (FacePanel *)emotionManagerView {
    if (!_emotionManagerView) {
        FacePanel *emotionManagerView = [[FacePanel alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds), CGRectGetWidth(self.view.bounds), self.keyboardViewHeight)];
...
- (void)initilzer {
    ...
    CGFloat inputViewHeight = (self.inputViewStyle == XHMessageInputViewStyleFlat) ? 60 : 55;
...
//    self.automaticallyAdjustsScrollViewInsets = YES;
   ...
   XHMessageTableView *messageTableView = [[XHMessageTableView alloc] initWithFrame:self.view.bounds style:UITableViewStylePlain];
    ...
    // 初始化输入工具条
    XHMessageInputView *inputView = [[XHMessageInputView alloc] init];
        ...
        [UIView animateWithDuration:0.25f
                         animations:^{
                             ...
                             if (_previousTextViewContentHeight) {
                                 ...
                                 self.messageInputView.frame = CGRectMake(0.0f,
                                                                          inputViewFrame.origin.y -16,
                                                                          inputViewFrame.size.width,
                                                                          inputViewFrame.size.height + 16);
                             }
                             ...
                             if (!isShrinking) {
                                 ...
                                 if (_previousTextViewContentHeight) {
                                     ...
                                     [self.messageInputView adjustTextViewHeightBy:16];
                                 }
...
- (void)startRecord {
    ...
    //检测是否开启麦克风
    if (![XHVoiceCheckHelper recordCheck]) {
        return;
    }
...
- (void)recordingFinishedWithFileName:(NSString *)filePath time:(NSTimeInterval)interval {
    ...
    if (interval >= 0.5 && interval < 61) {
       
        [self didSendMessageWithVoice:filePath voiceDuration:[NSString stringWithFormat:@"%.1lf",interval]];
        return;
    }
...
//音量大小
- (void)peakPowerForChannel:(float)peakPower{
...
- (void)didSendTextAction:(NSString *)text {
    ...
    if ([self.delegate respondsToSelector:@selector(didSendText:fromSender:onDate:)]) {
        [self.delegate didSendText:text fromSender:self.messageSender onDate:[NSDate date]];
        CGRect rect = self.messageInputView.inputTextView.frame;
//        rect.origin.y = 4.5;
        rect.size.height = [XHMessageInputView textViewLineHeight];
        self.messageInputView.inputTextView.frame = rect;
        self.messageInputView.inputTextView.center = CGPointMake(self.messageInputView.inputTextView.center.x, self.messageInputView.faceSendButton.center.y);
    }
}
...
- (void)facePanelSendTextAction:(FacePanel *)facePanel
{
    if ([self.messageInputView.inputTextView.text length]) {
        ...
        [self didSendText:self.messageInputView.inputTextView.text fromSender:self.messageSender onDate:[NSDate date]];
    }
...
//    self.messageTableView.frame = CGRectMake(self.messageTableView.frame.origin.x, 0, self.messageTableView.frame.size.width, self.view.frame.size.height);
//
//    if (self.isPresenting) {
//
//        self.messageTableView.frame = CGRectMake(self.messageTableView.frame.origin.x, 0, self.messageTableView.frame.size.width, self.view.frame.size.height-44);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XHMessage * message = [self.dataSource messageForRowAtIndexPath:indexPath];
