...
- (void)loadSystemEmoji:(NSArray *)faceData
{
    for (int i = 0; i < faceData.count; i++) {
        ...
        btn.titleLabel.font = [UIFont systemFontOfSize:28.f];
...
{
    FaceModel *model = faceData.firstObject;
    FaceButton *button = self.buttons.firstObject;
    CGFloat s = 1;
    if (button &&model.faceIcon) {
        UIImage *faceImage = [UIImage imageNamed:model.faceIcon];
        CGFloat maxImageWH = MAX(faceImage.size.width, faceImage.size.height);
        CGFloat minBtnWH = MIN(button.frame.size.width, button.frame.size.height);
        s = (minBtnWH-5)/maxImageWH;
    }
    ...
    for (int i = 0; i < faceData.count; i++) {
        ...
        FaceButton *btn = self.buttons[i];
        btn.hidden = NO;
        UIImage *image = [UIImage imageNamed:fm.faceIcon];
        [btn setImage:[image jj_scaleToSize:CGSizeMake(image.size.width*s, image.size.height*s)] forState:UIControlStateNormal];
        btn.faceTitle = fm.faceTitle;
    }
...
- (void)loadGifEmoji:(NSArray *)faceData
{
    for (int i = 0; i < faceData.count; i++) {
        ...
        FaceButton *btn = self.buttons[i];
        btn.hidden = NO;
//        [btn setBackgroundImage:[UIImage imageNamed:fm.faceIcon] forState:UIControlStateNormal];
