...
- (void)configureMessagePhoto:(UIImage *)messagePhoto thumbnailUrl:(NSString *)thumbnailUrl originPhotoUrl:(NSString *)originPhotoUrl onBubbleMessageType:(XHBubbleMessageType)bubbleMessageType {
    ...
    self.messagePhoto = messagePhoto;
     self.messageImageView.image = [[UIImage imageNamed:@"SenderTextNodeBkg"] stretchableImageWithLeftCapWidth:50 topCapHeight:30];
    if (originPhotoUrl) {
        
        self.messagePhoto = [messagePhoto jj_changeAlpha:0.2];
    }
    
    if (thumbnailUrl) {
        WEAKSELF
//         _activityIndicatorView.center = CGPointMake(140/ 2.0, 140 / 2.0);
...
//        [self.activityIndicatorView startAnimating];
        ...
        [self setImageWithURL:[NSURL URLWithString:thumbnailUrl] placeholer:nil showActivityIndicatorView:YES completionBlock:^(UIImage *image, NSURL *url, NSError *error) {
...
- (void)setupBackGroundImageView:(XHBubbleMessageType)messageType{
    
      UIImageView *imageView = [UIImageView new];
    imageView.frame = self.frame;
//        if (messageType == XHBubbleMessageTypeSending) {
//            imageView.image = [[UIImage imageNamed:@"weChatBubble_Sending_Solid"] stretchableImageWithLeftCapWidth:50 topCapHeight:30];
//        }else{
//            imageView.image = [[UIImage imageNamed:@"weChatBubble_Receiving_Solid"] stretchableImageWithLeftCapWidth:50 topCapHeight:30];
//        }
    UIImage *image = [[UIImage jj_imageWithColor:[UIColor colorWithHexString:kJMMainColorHexString] size:CGSizeMake(100, 50)] imageByRoundCornerRadius:8];
    imageView.image = [image stretchableImageWithLeftCapWidth:50 topCapHeight:30];
...
- (void)setupSubViewFrame:(CGRect )frame{
    ...
    if (![[NSValue valueWithCGRect:frame] isEqualToValue:[NSValue valueWithCGRect:CGRectZero]]) {
        _messageImageView.frame = CGRectMake(0, 0, CGRectGetWidth(frame), CGRectGetHeight(frame));
        if (self.messagePhoto) {
            [self setupBackGroundImageView: self.bubbleMessageType];
            self.messageImageView.image = self.messagePhoto;
            [self.messageImageView setNeedsDisplay];
        }
    }
