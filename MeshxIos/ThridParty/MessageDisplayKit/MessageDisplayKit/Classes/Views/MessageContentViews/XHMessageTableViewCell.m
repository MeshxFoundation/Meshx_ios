...
static const CGFloat  radius = 5.f;
...
static const CGFloat kXHAvatarPaddingY = 5;

static const CGFloat kXHUserNameLabelHeight = 15;
...
- (void)copyed:(id)sender {
    [[UIPasteboard generalPasteboard] setString:self.messageBubbleView.mlLabel.text];
...
- (void)configureTimestamp:(BOOL)displayTimestamp atMessage:(XHMessage *)message {
    ...
    if (displayTimestamp) {
          ...
          self.timestampLabel.text = [XHDateManager getTimeWithDate:message.timestamp];
        return;
        ...
        if (dateComponents.year == todayComponents.year && dateComponents.month == todayComponents.month && dateComponents.day == todayComponents.day) {
            dateText = NSLocalizedStringFromTable(@"Today", @"MessageDisplayKitString", @"Today");
            ...
            dateText = NSLocalizedStringFromTable(@"Yesterday", @"MessageDisplayKitString", @"Yesterday");
        } else {
...
- (void)configAvatarWithPhoto:(UIImage *)photo {
    [self.avatarButton setImage:[photo jj_imageAddCornerWithRadius:CGRectGetWidth(self.avatarButton.frame)/2.0 size:self.avatarButton.frame.size]forState:UIControlStateNormal];
}
...
- (void)setPressGestureRecognizeWithMessage:(XHMessage *)message{
...
- (instancetype)initWithMessage:(XHMessage *)message
    ...
    if (self) {
        ...
        if (!_timestampLabel) {
            ...
            if (!timestampBackgroundColor) {
                timestampBackgroundColor = [UIColor colorWithWhite:0.8 alpha:1.000];
            }
            LKBadgeView *timestampLabel = [[LKBadgeView alloc] initWithFrame:CGRectMake(0, kXHLabelPadding-5, MDK_SCREEN_WIDTH, kXHTimeStampLabelHeight)];
        ...
        if (message.shouldShowUserName) {
            // 3、配置用户名
            UILabel *userNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetWidth(avatarButton.frame), CGRectGetMinX(avatarButton.frame)-kXHUserNameLabelHeight, CGRectGetWidth(self.avatarButton.bounds) + 20, kXHUserNameLabelHeight)];
    ...
    //设置长按手势
    [self setPressGestureRecognizeWithMessage:message];
...
- (void)layoutSubviews {
    ...
    // 布局头像
    CGFloat layoutOriginY = kXHAvatarPaddingY + (self.displayTimestamp ? kXHTimeStampLabelHeight : -kXHAvatarPaddingY);
...
//    CGFloat timeStampLabelNeedHeight = (self.displayTimestamp ? (kXHTimeStampLabelHeight + kXHLabelPadding) : 0);
    ...
    CGFloat timeStampLabelNeedHeight = (self.displayTimestamp ? self.avatarButton.frame.size.height/2.0-diff:-kXHAvatarPaddingY-diff);
    self.avatarButton.hidden = NO;
    self.userNameLabel.hidden = NO;
    CGRect bubbleMessageViewFrame = CGRectZero;
    if (self.messageBubbleView.message.shouldShowUserName) {
        ...
        if ([self bubbleMessageType] == XHBubbleMessageTypeReceiving) {
            ...
            self.userNameLabel.frame = CGRectMake(bubbleX+12,CGRectGetMinY(self.avatarButton.frame) ,CGRectGetWidth(self.contentView.bounds) - bubbleX - offsetX , kXHUserNameLabelHeight);
            bubbleMessageViewFrame = CGRectMake(bubbleX,
                                                timeStampLabelNeedHeight+self.userNameLabel.frame.size.height,
            ...
            self.userNameLabel.frame = CGRectMake(bubbleX-10,CGRectGetMinY(self.avatarButton.frame),CGRectGetWidth(self.contentView.bounds) - bubbleX - offsetX , kXHUserNameLabelHeight);
            bubbleMessageViewFrame = CGRectMake(bubbleX,
...
//    self.messageBubbleView.emotionImageView.animatedImage = nil;
    self.messageBubbleView.emotionImageView.image = nil;
