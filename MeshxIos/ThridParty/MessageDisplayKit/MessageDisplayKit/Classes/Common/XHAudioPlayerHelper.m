...
- (void)managerAudioWithFileData:(NSData *)amrData toPlay:(BOOL)toPlay{

    if (toPlay) {
        [self playAudioWithFileData:amrData];
    } else {
        [self pausePlayingAudio];
    }
}
...
- (void)playAudioWithFileData:(NSData *)fileData{
    ...
    if ([fileData length] > 0) {
        ...
        if (_playingFileData && [fileData isEqualToData:_playingFileData]) {//上次播放的录音
            if (_player) {
                [_player play];
             ...
             */
            AVAudioPlayer *pl = [[AVAudioPlayer alloc] initWithData:fileData error:nil];
            pl.delegate = self;
            [pl play];
            self.player = pl;
            [[UIDevice currentDevice] setProximityMonitoringEnabled:YES];
