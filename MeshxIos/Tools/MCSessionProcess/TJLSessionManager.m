...
#define ServiceType @"JMSTService"
...
@property(nonatomic, copy) void(^lostPeer)(MCNearbyServiceBrowser *browser, MCPeerID *peerID);
...
- (instancetype)initWithDisplayName:(NSString *)displayName securityIdentity:(NSArray *)security encryptionPreferences:(MCEncryptionPreference)preference serviceType:(NSString *)type {
    ...
    self.session = [[MCSession alloc]initWithPeer:self.peerID securityIdentity:security encryptionPreference:preference];
...
- (void)lostPeerWithBrowser:(void (^)(MCNearbyServiceBrowser *, MCPeerID *))lost{
    self.lostPeer = lost;
}
